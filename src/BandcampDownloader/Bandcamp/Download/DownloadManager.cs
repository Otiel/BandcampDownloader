...
internal sealed class DownloadManager : IDownloadManager
{
    ...
    private async Task DownloadAlbumAsync(Album album, CancellationToken cancellationToken)
    {
        ...
        {
            artworkStream = await DownloadCoverArtAsync(album, cancellationToken);
        }
        ...
        await Task.WhenAll(indexes.Select(async i => tracksDownloaded[i] = await DownloadAndTagTrackAsync(album, album.Tracks[i], artworkStream, cancellationToken)));

        // Cleanup
        if (artworkStream != null)
        {
            await artworkStream.DisposeAsync();
        }
        ...
        do
        {
            ...
            if (trackDownloaded)
            {
                ...
                if (_userSettings.SaveCoverArtInTags && artworkStream != null)
                {
                    ...
                    await _tagService.SaveCoverInTrackAsync(track, inTagsArtworkStream, cancellationToken);
